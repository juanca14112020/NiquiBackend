...
public class AuthService : IAuthService
{
    ...
    public async Task<Guid> RegisterSuperAdminAsync(RegisterRequestDto request, Guid createrdByDeveloperId )
    {
        var developerExists = await _context.Developers.AnyAsync(d => d.DeveloperId == createrdByDeveloperId);
        ...
        var superAdmin = new SuperAdmin
        {
            ...
            PasswordHash = _passwordHasher.Hash(request.Password),
            IsActive = true,
            CreatedByDeveloperId = createrdByDeveloperId
        };
    ...
    public async Task<Guid> RegisterAdminAsync(RegisterRequestDto request, Guid? createdByDeveloperId, Guid? createdBySuperAdminId)
    {
        if (createdByDeveloperId is null && createdBySuperAdminId is null)
            throw new InvalidOperationException("Debe indicar quíen crea el Admin (Developer o SuperAdmin)");
